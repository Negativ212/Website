const newsData = [
    { id: 1, title: "Lorem ipsum dolor sit amet", date: "14 мая 2026", excerpt: "Consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.", fullText: "Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur.", image: "https://i.imgur.com/placeholder1.jpg" },
    { id: 2, title: "Sed do eiusmod tempor", date: "10 мая 2026", excerpt: "Incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud.", fullText: "Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.", image: "https://i.imgur.com/placeholder2.jpg" },
    { id: 3, title: "Duis aute irure dolor", date: "5 мая 2026", excerpt: "In reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur.", fullText: "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.", image: "https://i.imgur.com/placeholder3.jpg" },
    { id: 4, title: "Excepteur sint occaecat", date: "28 апреля 2026", excerpt: "Cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.", fullText: "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium.", image: "https://i.imgur.com/placeholder4.jpg" }
];
...
function updateCharacterCard(index) {
    ...
    if (detailsDiv && !window.detailsVisible) detailsDiv.classList.add('hidden');
...
function renderThumbnails() {
    ...
    document.querySelectorAll('.thumbnail-item').forEach(item => {
        item.addEventListener('click', () => {
            ...
            window.currentCharIndex = idx;
            updateCharacterCard(window.currentCharIndex);
            const detailsDiv = document.getElementById('characterDetails');
            if (detailsDiv) detailsDiv.classList.add('hidden');
            window.detailsVisible = false;
        });
    });
    updateCharacterCard(window.currentCharIndex || 0);
}
...
function initDetailsButton() {
    ...
    btn.addEventListener('click', () => {
        const char = charactersData[window.currentCharIndex || 0];
        document.getElementById('charFullDesc').innerText = char.desc;
        detailsDiv.classList.toggle('hidden');
        window.detailsVisible = !detailsDiv.classList.contains('hidden');
    });
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (quizForm) {
        quizForm.addEventListener('submit', function(e) {
            ...
            if (total <= 6) {
                character = 'Айден, Охотник на нежить';
                description = 'Ты смел и решителен, не боишься смотреть смерти в лицо.';
            } else if (total <= 10) {
                character = 'Лоремиус, Магистр Ордена';
                description = 'Ты мудр и рассудителен, предпочитаешь знания грубой силе.';
            } else if (total <= 14) {
                character = 'Моргана, Призрачная леди';
                description = 'Ты загадочен(на) и гибок, танцуешь между светом и тьмой.';
            } else {
                character = 'Велиар, Тёмный лорд';
                description = 'Ты жаждешь власти и не остановишься ни перед чем.';
            }
            document.getElementById('resultCharacter').innerText = character;
    ...
    class QuoteManager {
        ...
        async loadLocalQuotes() {
            ...
            try {
                ...
                let quotesArray = data.quotes && Array.isArray(data.quotes) ? data.quotes : (Array.isArray(data) ? data : []);
                ...
                if (this.localQuotesCache.length === 0) throw new Error();
